...
export async function createLevel(gameId: string, description: string): Promise<LevelType> {
    const [lastLevel] = await db.select({ number: levels.number }).from(levels).where(eq(levels.gameId, gameId)).orderBy(desc(levels.number)).limit(1);

    const number = (lastLevel?.number ?? 0) + 1;
...
export async function getLevel(number: number): Promise<LevelType | undefined> {
    const [level] = await db.select().from(levels).where(eq(levels.number, number)).limit(1);
